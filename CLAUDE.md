...
- **Redirecting back to where the user came from after login: pass `state={{ from: location.pathname + location.search }}` on any `Link` to `/login`** (`useLocation()` from `react-router`), and have `LoginPage` read `location.state?.from` (default `/`) to `navigate(from, { replace: true })` on success instead of always going home — `{ replace: true }` keeps `/login` out of back-button history. The same `from` is threaded through the register↔login cross-links too (`RegisterPage`'s links to `/login`, `LoginPage`'s link to `/register` both pass `state={{ from }}` forward), so a detour through registration doesn't lose the original destination. See `Header.tsx`/`ComicReadingSources.tsx` for the two current entry points into `/login`.
