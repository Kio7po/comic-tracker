...
- **Decidido — `ComicReadingSource` también es aportación comunitaria, no dato curado como `ComicMetadataSource`:** un usuario puede proponer un sitio nuevo al aportar una entry, no solo elegir uno ya registrado. Por eso tiene el mismo ciclo `PENDING`/`APPROVED`/`REJECTED` y las mismas columnas de auditoría (`created_at`/`updated_at`/`reviewed_at`/`reviewed_by`) que `ComicReadingEntry`, más `contributed_by`. Consecuencia directa: aprobar una `ComicReadingEntry` exige que su `ComicReadingSource` ya esté `APPROVED` — no tiene sentido validar el enlace de un sitio que en sí mismo no se ha verificado todavía.
  - **Decidido — rechazar una `ComicReadingSource` rechaza en cascada sus `ComicReadingEntry` todavía `PENDING`:** consecuencia simétrica de la anterior — si el sitio en sí queda `REJECTED`, ninguna de sus entries pendientes podrá aprobarse nunca (por la misma regla), así que dejarlas en `PENDING` las abandonaría indefinidamente en vez de resolverlas. Las que ya estuvieran `APPROVED` o `REJECTED` no se tocan.
...
- **Decidido — el icono de una fuente no lo aporta el usuario, se resuelve automáticamente:** mismo criterio que `title`/`available_chapters` en `ComicReadingEntry` (dato hidratable, no exigido en el formulario). El mecanismo es un puerto (`ComicReadingSourceIconResolver`, detectado por dominio, mismo espíritu que `ComicReadingProvider`) para poder añadir resolutores específicos por sitio más adelante sin tocar el resto del flujo.
  - **Decidido — el resolutor por defecto no apunta a `/favicon.ico` del propio sitio, sino al proxy de favicons de Google (`google.com/s2/favicons?domain=...`).** *Razón:* algunos sitios envían cabecera `Cross-Origin-Resource-Policy` en sus assets estáticos, que bloquea al navegador cargar ese icono como subrecurso cross-origin desde un `<img>` — el bloqueo no depende de CORS clásico, ocurre igual. Enrutar a través de Google evita el bloqueo (la petición va a `google.com`, sin esa cabecera) y de paso resuelve sitios con el favicon en una ubicación o formato no estándar, ya cubierto por el propio rastreo de Google. *Contrapartida asumida:* introduce una dependencia de un servicio de terceros para algo antes autocontenido — se acepta por ser la misma técnica que ya usaba el prototipo original de esta pantalla antes de existir este resolutor en el backend.
- **Decidido — listar las `ComicReadingEntry` de un cómic no oculta las `PENDING` por defecto:** si un usuario aporta una fuente y no la ve aparecer en ningún sitio, parece que la aportación no sirvió de nada. El filtrado por estado queda como opción del llamador, no como comportamiento forzado. En la práctica, el frontend lo materializa como un checkbox en la ficha del cómic: sin marcar, solo se muestran las `APPROVED`; marcado, también las `PENDING`. Las `REJECTED` no se muestran ahí nunca, independientemente del checkbox — ya se descartaron explícitamente, así que no aportan valor mostrarlas de forma pública.
...
- **Decidido — el selector de fuente existente (elegir `sourceId` en vez de proponer una nueva) debe incluir fuentes `PENDING`, no solo `APPROVED`:** limitarlo a `APPROVED` dejaría invisible, para cualquier otro colaborador, una fuente recién propuesta y aún sin revisar — empujaría a proponerla de nuevo como si fuera nueva, generando duplicados evitables por la propia limitación del selector. Las `REJECTED` sí se excluyen siempre, al estar ya descartadas explícitamente.
- **Decidido — listar `ComicReadingSource`/`ComicReadingEntry` por estado es un único endpoint parametrizado por la lista completa de estados a incluir, no un endpoint con un filtro opcional que cambia de comportamiento según si se manda o no.** Se descartó explícitamente la alternativa (un `status` opcional donde su ausencia dispara un comportamiento por defecto distinto, p. ej. "todo menos `REJECTED`") por acoplar dos casos de uso distintos al mismo método con una rama oculta — el llamador declara siempre explícitamente qué quiere ver (`statuses=APPROVED,PENDING` para el selector de fuentes, `statuses=PENDING` para moderación), sin comportamiento implícito que adivinar. El orden (`sortBy`/`direction`) se decidió con el mismo criterio: parámetros explícitos con un valor por defecto razonable, no una decisión tomada por el backend sin que el llamador pueda cambiarla.
- **Decidido — la moderación de `ComicReadingEntry` es una vista cruzada entre cómics, paginada y solo para `ADMIN`, a diferencia del listado de `ComicReadingSource` (público, sin paginar).** Dos razones distintas para cada diferencia: (a) las entries son muchas más que las fuentes por naturaleza — cada fuente puede tener entries en decenas de cómics — así que sin paginación la respuesta crecería sin límite; (b) no existe un caso de uso anónimo legítimo para "ver todas las aportaciones pendientes de todos los cómics" del modo en que sí lo hay para el selector de fuentes (elegir una fuente ya conocida al proponer un enlace) — es una pantalla exclusivamente de moderación.
  - **Decidido — la respuesta de la moderación de entries no reutiliza/enriquece el DTO del listado por cómic, sino que lo envuelve en uno nuevo** (`ComicReadingEntryModerationResponseDto(entry, comic)`). *Razón:* el listado por cómic no necesita saber a qué cómic pertenece cada entry (ya lo sabe el llamador, viene en la URL) y su relación `comic` se queda `LAZY` sin cargar; la vista de moderación sí lo necesita (cruza cómics) y su propio finder pide esa relación con `@EntityGraph` a propósito. Compartir un único DTO enriquecido habría acoplado el caso común a una carga ansiosa que no necesita, o dependido silenciosamente de que Hibernate reutilice una entidad ya cargada en el mismo contexto de persistencia para no pagar el N+1 — un mecanismo real pero implícito, no una garantía explícita a nivel de consulta.
  - **Pendiente, aparcado por tiempo:** filtrar la moderación de entries por fuente o por cómic concretos (útil para un admin que solo quiere revisar las aportaciones de una fuente que ya conoce). El endpoint ya acepta estados/orden/paginación de forma explícita, así que añadir estos dos filtros más adelante es extender los mismos query params, no rediseñar el endpoint.
- **Decidido — quién aportó una `ComicReadingSource`/`ComicReadingEntry` (`contributedBy`) solo se expone en las vistas de moderación, nunca en los listados públicos que devuelven esas mismas entidades** (el listado por cómic, el selector de fuentes). *Razón:* es preferible intentar que la API no exponga la identidad de otro usuario a cualquiera sin autenticar — ningún endpoint lo hace hoy en ningún otro sitio.
...
- **Decidido — estados de `ReadingState`: `READING`, `COMPLETED`, `ON_HOLD`, `PLAN_TO_READ`, `DROPPED`** (mismo criterio que MyAnimeList), formalizando el "leyendo, completado, en pausa, quiero leer" que `domain.mmd` había dejado sin cerrar como enum concreto. No se incluye un estado de releer (`REPEATING` en MAL): no se consideró necesario para el alcance del TFG.
- **Decidido — un usuario tiene como mucho un `ReadingState` por cómic** (`UNIQUE(comic_id, user_id)`): no tiene sentido llevar dos progresos distintos sobre la misma obra para el mismo usuario.
- **Decidido — `ReadingState` sí lleva `created_at`/`updated_at`**, a diferencia de lo que reflejaba `domain.mmd` hasta ahora (solo `status`/`chapters`). *Razón:* `updated_at` informa de cuándo actualizó el usuario su progreso por última vez, un dato con valor propio de cara al usuario (p. ej. poder ordenar su lista de seguimiento por "actualizado recientemente").
- **Decidido — actualizar el progreso es una única operación que reemplaza el estado completo (`status` + `chapters`), no un endpoint de incremento/decremento por cada pulsación de +/-.** *Razón:* los controles +/- (ver granularidad del progreso, arriba) son un patrón de interacción del frontend sobre un valor en memoria, no justifican una llamada de red por click — mandar una petición por cada capítulo sería un mal uso de la red. El formulario de edición (ver Frontend) manda el estado nuevo completo con un único guardado.
- **Decidido — el listado de `ReadingState` por usuario no reutiliza ni el DTO de cómic completo ni el resumen público (`ComicSummaryResponseDto`, pensado para catálogo/moderación), sino un DTO propio (mismo patrón de envoltorio ya usado en moderación de entries) con solo los campos que la pantalla de biblioteca personal necesita para filtrar/ordenar/mostrar.** *Razón:* el DTO completo forzaría cargar ansiosamente las relaciones `LAZY` autores/géneros/etiquetas en cada fila de un listado que crece con el uso, sin que esa pantalla las use nunca; el resumen público tampoco basta porque le faltan campos que aquí sí hacen falta (capítulos, títulos alternativos, tipo, estado, fecha de inicio, nsfw). El propio finder de este listado amplía su `@EntityGraph` para cubrir también la colección de títulos alternativos (otra relación `LAZY` de `Comic`, no cubierta solo con cargar la asociación `comic`), mismo criterio de "cargar ansiosamente lo que esta consulta concreta necesita" que ya se aplicaba a otros finders de listado.
...
- **Decidido — revisar (`approve`/`reject`) una `ComicReadingEntry` o `ComicReadingSource` exige rol `ADMIN`:** primera regla de autorización por rol del proyecto (`hasRole(...)` en `SecurityConfig`), apoyada en el claim `role` que el JWT ya incluye desde su emisión. Misma regla aplicada a ambas entidades, no dos decisiones independientes. El resto del alcance de administración sigue sin concretar.
